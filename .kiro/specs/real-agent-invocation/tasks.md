...
- [x] 2.4 実接続モードの起動時認証検証と設定の受け渡しを実装する
...
- [x] 3.2 (P) エージェント呼び出し失敗の HTTP エラー応答を追加する
...
- [x] 4.2 (P) エージェント到達ルート5関数をスレッドプール実行に切り替える
...
- [x] 5.2 実 Gemini での手動スモークスクリプトを追加し、提出前に実行する
  ...
  - _Result: 2026-07-06 に Google Cloud ADC + Vertex AI モードで local backend server を起動し、API 経由で4操作1周に成功。結果は `.kiro/specs/real-agent-invocation/manual-smoke-result.json` に記録済み。_
