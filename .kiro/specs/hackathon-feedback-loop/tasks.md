...
- [x] 1. Foundation: データ契約の拡張
- [x] 1.1 backend の API schema に改善ループ用の field と型を追加する
...
- [x] 1.2 (P) agent パッケージの契約と設定を拡張する
...
- [x] 2. 出題観点（drillFocus）の backend 管理
- [x] 2.1 講座の作成・更新で出題観点を保存し version 管理する
...
- [x] 2.2 ドリル生成時に出題観点を snapshot して agent へ渡す
...
- [x] 3. 根拠付きドリル生成の保証
- [x] 3.1 生成結果の根拠検証を backend 境界に追加する
...
- [x] 3.2 (P) ドリル生成プロンプトを観点優先・原文引用に更新する
...
- [x] 3.3 (P) local 実行モードの生成 fallback を教材準拠にする
...
- [x] 4. 分析前の採点集計（scoreSummary）を管理応答に追加する
...
- [x] 5. 分析タイムラインの backend 段階保存
- [x] 5.1 分析実行を固定 5 step で段階記録する
...
- [x] 6. 誤答分析の多視点化（composed agent）
- [x] 6.1 3 レンズ並列 + 統合の composite agent を作る
  ...
  - _Depends: 1.2_
- [x] 6.2 composite 対応で既存テストと eval を更新する
...
- [x] 8.2 (P) Course Editor に出題観点入力を追加する
  ...
  - _Depends: 8.1_
- [x] 8.3 (P) 分析タイムライン表示 component を作る
  ...
  - _Depends: 8.1_
- [x] 8.4 Drill Admin に採点集計と分析 polling を追加する
...
- [x] 9. 統合検証
- [x] 9.1 受講者秘匿と改善ループの integration テストを追加する
...
- [x] 9.2 全テストスイートと agent eval で回帰確認する
