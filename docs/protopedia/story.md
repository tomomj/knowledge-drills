...
上の経費精算デモはシードデータによる再現です。ここには、実在の人の回答で回った改善ループを、
日付つきで**追記だけ**していきます。提出後も本番の運用は止まりません。
このセクションの行が増えていくこと自体が、「生きているドキュメント」という主張の実演です。

- **2026-07-12（提出時点）**: 実在の回答【N】件で 1 周目を実行。誤答は【つまずき箇所】に集中し、
  分析エージェントが起案したパッチを適用して教材 v2 を公開。
- **2026-07-19（予定）**: v2 への実回答で平均スコアを再観測し、Before / After をここに追記。

（回答が早く集まった場合は、予定を待たず前倒しで追記します）
