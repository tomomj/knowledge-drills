...
「却下からの学習」は引き続き残る。invoker と schema の信頼境界は変えずに、分析入力へ
人間の判断材料を足すことで実現できる見立てだ。

一方、この周回で誤答の蓄積を要分析バッジとして知らせる alerting まで実装した。
**検知は自動化した。次は起動（分析の自動実行）の自動化**だ。現状は講座オーナーがバッジを見て
分析ボタンを押している。次は閾値超過をトリガーに分析を起動し、提案を人間のレビュー待ちへ
届けたい。patch apply の人間ゲートは残したまま、検知から分析開始までの待ち時間をなくす。
これを主たる次段階とし、却下理由のフィードバックも並行して育てる。
