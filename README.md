...
| SLO 改善の確認 | Before / After 平均点（v1 5.4 → v3 10.8、12点満点） |

## 2 分で改善ループを一周する

ログインすると回答データ投入済みのデモ講座が用意されます。

1. **デモ講座を開く** — 採点済み回答 2 件と設問別のつまずきが見える
2. **受講者として回答する** — 共有 URL からログイン不要で回答・AI 採点
3. **分析エージェントを実行** — 実行状況がタイムラインで流れ、パッチ要否を自律判断
4. **提案されたパッチをレビューして適用** — 判断ログと diff を確認して、適用または却下する
5. **スコアの推移で効果を確認** — 教材 v1 平均 5.4 点 → v3 平均 10.8 点（12点満点）。改善が数字で閉じる
...
- **見送り判断**: 分析エージェントは「パッチを出さない」判断を含めて自律的に決めます。
  critic / reviewer が所見を 1 件も承認しなかった場合はパッチ起案を見送り、
  その判断は分析タイムラインに記録されます。却下されたパッチの理由を次回分析に
  還元する「却下からの学習」は次の拡張です
